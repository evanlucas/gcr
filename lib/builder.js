...
Builder.prototype.setup = function(opts) {
  ...
  this.currentBuild = {
    ...
    , timeout: +(gcr.config.get('timeout') || opts.timeout || 5000) * 1000
...
Builder.prototype.append = function(str) {
  if (this.currentBuild) {
    this.currentBuild.output += str
  } else {
    log.warn('[builder]', 'trying to append stdout/stderr to a job that has '+
      'already finished', '['+str+']')
    log.warn('[builder]', 'This is a bug. Please open an issue on '+
      'https://github.com/evanlucas/gcr/issues')
  }
}
...
Builder.prototype.run = function() {
  ...
  if (!len && gcr.config.get('npm')) {
    this.currentBuild.commands.push('npm install')
    this.currentBuild.commands.push('npm test')
  }
